...
`starter.config` is the configuration composition layer for the `starter` library. It is the only subsystem permitted to
...
Before Hydra composition, `starter.config` loads environment values from:
...
```python
from starter.config import load_env_files
...
```bash
starter logging=console tracking=disabled runtime.seed=42
```
