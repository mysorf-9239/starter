...
> Configuration is composed once. Subsystem instances are constructed once. Both are delivered to application code through an immutable `RuntimeContext`.
...
`starter.sweeps` operates independently of the bootstrap sequence. A `RuntimeContext` is passed in to provide sweep-level tracking and artifact management.
...
When `bootstrap()` is not appropriate, subsystems can be constructed individually by passing the relevant config section:
...
| Reading `os.environ` for credentials in domain code | Use `oc.env` in config YAML |
